...
deleted`TINYINT DEFAULT 0COMMENT‘逻辑删除标志(0-未删除,1-已删除)’,
create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
update time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE
CURRENT_TIMESTAMP,
...
create time` DATETIME DEFAULT CURRENT TIMESTAMP,
...
`next due date`DATE NOT NULL COMMENT‘下次接种日期’,
...
`notes`TEXT COMMENT‘备注’,
`deleted`TINYINT DEFAULT0COMMENT‘逻辑删除标志(0-未删除,1-已删除)’,
create time` DATETIME DEFAULT CURRENT_TIMESTAMP,
FOREIGN KEY(`pet_id`) REFERENCES`pet`(`pet_id`) ON DELETE CASCADE,
INDEX`idx_pet_id`(pet_id`)
...
business hours`VARCHAR(100) COMMENT‘营业时间’
deleted`TINYINT DEFAULT 0 COMMENT‘逻辑删除标志(0-未删除,1-已删除)’,
create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
...
数据库初始化脚本-宠物健康管理系统--包含:宠物品种字典和测试用户数据-
--1.创建宠物品种字典表并插入数据CREATE TABLE IF NOT EXISTS`pet_breed`(
...
--插入常见犬种INSERT INTO`pet_breed`(`species`,`breed_name`) VALUES
...
--插入常见猫种INSERT INTO`pet_breed`(`species`,`breed_name`) VALUES
(猫，英国短毛猫),(猫，美国短毛猫),(猫，’布偶猫’),(‘猫’,‘遥罗猫’),(‘猫’,‘波斯猫’),
(猫，苏格兰折耳猫),(猫，俄罗斯蓝猫),(猫，孟加拉豹猫),(猫，挪威森林猫),(‘猫’,‘缅因猫’),
(猫，埃及猫),(猫，怕曼猫),(猫，’异国短毛猫’),(猫’,‘狸花猫’),(‘猫’,‘三花猫’),
(猫，’橘猫’),(‘猫’,‘黑猫’),(‘猫’,‘白猫’),(‘猫’,‘其他’);
--2.插入测试用户数据INSERT INTO`user`(`openid`,`phone`,`password`,`nickname`,`avatar_url`)VALUES
(‘test_openid_001’，‘13800138001’，‘$2a$10$rT7QqL7Xq9Q8qW8qW8qW8u’，‘宠物主人张’，‘https://example.com/avatar1.jpg’),
(‘test_openid_002’，‘13800138002’，‘$2a$10$rT7QqL7Xq9Q8qW8qW8qW8u’，‘爱宠达人李’‘https://example.com/avatar2.jpg’),
(test_openid_003’，‘13800138003’，‘$2a$10$rT7QqL7Xq9Q8qW8qW8qW8u’,‘猫咪守护者’,‘https://example.com/avatar3.jpg’);
--3.插入测试宠物数据INSERT INTO`pet`(`user_id`,`name`,`species`,`breed`,`birth_date`,`gender`,`weight`,`neutered_status`,`avatar_url`) VALUES
...
-4.插入疫苗记录示例INSERT INTO`vaccination_record`(`pet_id`,`vaccine_name`,vaccination_date`,`next_due_date`,`vet_info`) VALUES
(1，’狂犬疫苗’，2024-12-01’，‘2025-12-01’，‘张医生-爱心宠物医院’)，
(1，’六联疫苗’，2024-11-15’，‘2025-11-15’，‘李医生-动物保健中心’)，
(2，’猫三联’，‘2024-10-20’，‘2025-10-20’，‘王医生-猫咪专科医院’);
--5.插入驱虫记录示例INSERT INTO`deworming_record`(`pet_id`,`deworming_type`,`drug_name`,`date`,`next_date`) VALUES
(1，’体内’，‘拜宠清’，2024-12-01’，‘2025-01-01’)，
(1，’体外’，‘福来恩’，‘2024-12-01’，‘2025-01-01’),
(2,’体内’,‘海乐妙’,‘2024-11-20’,‘2024-12-20’);
--6.插入体检记录示例INSERT INTO`checkup_record`(`pet_id`,`checkup_date`,`institution`,result_summary`) VALUES
(1，‘2024-11-10’，‘爱心宠物医院’，‘身体健康，各项指标正常，建议保持当前饮食和运动量’)，
(2，‘2024-10-15’，‘猫咪专科医院’，‘体重偏轻，建议增加营养摄入’);
--7.插入病历记录示例 INSERT INTO`medical_record`(`pet_id`,`hospital`,`diagnosis`,medication_list`,`treatment_date`) VALUES
(1,’爱心宠物医院’,‘轻微皮肤病’,‘消炎药膏，每日涂抹两次’,‘2024-09-05’),
(2,’猫咪专科医院’,‘消化不良’,‘益生菌，每日一次’,‘2024-10-08’);
--8.插入提醒事项示例 INSERT INTO`reminder`(`user_id`,`pet_id`,`reminder_type`,`title`,`due_date`,`repeat_cycle`) VALUES
(1，1，‘疫苗’，‘豆豆的狂犬疫苗到期’，2025-12-01 09:00:00’，‘每年’)，
(1,1,‘驱虫’,‘豆豆的体内驱虫’,2025-01-01 09:00:00’,‘每月’),
(1,2,’疫苗’,‘咪咪的猫三联’,‘2025-10-2009:00:00’,‘每年’),
(1,NULL,‘自定义’,‘购买宠物粮食’,‘2024-12-25 10:00:00’,NULL);
--9.插入服务点示例数据INSERT INTO`service_point`(name`,`type`,`address`,`phone`,`latitude`,longitude`,`rating`,`business_hours`) VALUES
...
--10.插入成长事件示例(系统会自动生成，这里提供手动插入示例)INSERT INTO`growth_event(`pet_id`,`event_type`,`event_date`,`description`,`image_url`,`weight_value`) VALUES
...
-- =============================================-- 数据验证查询-- =============================================
-- 查看插入的数据SELECT ‘用户数量:’ as info, COUNT(*) as count FROM `user` UNION ALLSELECT ‘宠物数量:’, COUNT(*) FROM `pet` UNION ALLSELECT ‘品种数量:’, COUNT(*) FROM `pet_breed` UNION ALLSELECT ‘疫苗记录:’, COUNT(*) FROM `vaccination_record` UNION ALLSELECT ‘提醒事项:’, COUNT(*) FROM `reminder`;
-- 查看测试用户和宠物关联信息SELECT u.nickname as 用户, p.name as 宠物, p.species as 类型, p.breed as 品种 FROM `user` u LEFT JOIN `pet` p ON u.user_id = p.user_id WHERE u.deleted = 0 AND p.deleted = 0;